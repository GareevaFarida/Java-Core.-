...
public class Application {
    ...
    public int CheckMassive(String[][] massive) throws MyArraySizeException, MyArrayDataException {
        ...
        if (massive.length != x) {
            throw new MyArraySizeException("В метод передан массив с несоответствующей размерностью массива. Количество строк в массиве: " + massive.length + " вместо " + x);
        }

        //проверяем каждую строку массива на правильность количества элементов
        for (int i = 0; i < x; i++) {
            if (massive[i].length != y) {
                throw new MyArraySizeException("В метод передан массив с несоответствующей размерностью массива. Количество элементов в строке с индексом: " + i + " составляет " + massive[i].length + " вместо " + y);
            }
        }
